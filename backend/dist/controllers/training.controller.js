...
const normalizeStoredActivities = (raw) => {
    if (!Array.isArray(raw))
        return [];
    return raw.filter((item) => item && typeof item === 'object');
};
const mergeDetailsWithExistingActivities = (existingRaw, incomingRaw) => {
    const existing = parseDetails(existingRaw);
    const incoming = parseDetails(incomingRaw);
    const keys = new Set([...Object.keys(existing), ...Object.keys(incoming)]);
    const merged = {};
    keys.forEach((criterionId) => {
        const existingCriterion = existing[criterionId] && typeof existing[criterionId] === 'object'
            ? existing[criterionId]
            : {};
        const incomingCriterion = incoming[criterionId] && typeof incoming[criterionId] === 'object'
            ? incoming[criterionId]
            : {};
...
export const createTrainingScore = async (req, res) => {
    ...
    try {
        ...
        const secMaxPoints = [20, 25, 20, 25, 10]; // Sections I to V max points
        ...
        */
        ...
        WHERE id = ${existingScore.id}
      `;
            score = await prisma.trainingScore.findUnique({
                where: { id: existingScore.id },
                include: {
                    student: true,
                    semester: true,
            ...
            score = await prisma.trainingScore.create({
                data: {
                    student_id: targetStudentId,
                    semester_id: semesterName,
                    y_thuc: yt,
...
const getTrainingScoresLegacy = async (req, res) => {
    ...
    try {
        ...
        if (semester)
            where.semester_id = String(semester);
...
export const getTrainingScoreById = async (req, res) => {
    const { id } = req.params;
    try {
...
export const approveTrainingScore = async (req, res) => {
    const { id } = req.params;
    ...
    if (admin_details && typeof admin_details === 'object' && adminTotal === undefined) {
        adminTotal = Object.values(admin_details).reduce((sum, val) => sum + (Number(val) || 0), 0);
    }
    const criteriaMeta = Array.isArray(criteria_meta) ? criteria_meta : [];
    try {
        const updateData = {};
        if (status !== undefined)
            updateData.status = status;
        if (admin_y_thuc !== undefined)
            updateData.admin_y_thuc = Number(admin_y_thuc) || 0;
        if (admin_hoat_dong !== undefined)
            updateData.admin_hoat_dong = Number(admin_hoat_dong) || 0;
        if (admin_ky_luat !== undefined)
            updateData.admin_ky_luat = Number(admin_ky_luat) || 0;
        if (adminTotal !== undefined)
            updateData.admin_total = Number(adminTotal) || 0;
        if (admin_notes !== undefined)
            updateData.admin_notes = String(admin_notes);
        if (admin_details)
            updateData.admin_details = admin_details;
        ...
        let updated;
        try {
            const existing = await prisma.trainingScore.findUnique({
                where: { id: Number(id) }
            });
      ...
      `;
            ...
            updated = await prisma.trainingScore.findUnique({
                where: { id: Number(id) },
                include: {
                    student: true,
                    semester: true
...
export const getTrainingStatistics = async (req, res) => {
    const semesterName = normalizeSemesterName(req.query.semester);
    if (!semesterName) {
        return res.status(400).json({ message: 'Thieu hoc ky' });
    }
    try {
        const role = String(req.user?.role || '').toUpperCase();
        const requestedClassId = String(req.query.class_id || '').trim().toUpperCase();
        const effectiveClassId = role === 'BCH' ? String(req.user?.class_id || '').trim().toUpperCase() : requestedClassId;
        const studentWhere = effectiveClassId ? { class_id: effectiveClassId } : {};
        const scoreWhere = { semester_id: semesterName };
        if (effectiveClassId) {
            scoreWhere.student = { class_id: effectiveClassId };
        }
        const [students, scores] = await Promise.all([
            prisma.student.findMany({
                where: studentWhere,
        ...
        const scoreByStudentId = new Map();
        scores.forEach((score) => {
            scoreByStudentId.set(Number(score.student_id), score);
        });
        const classMap = new Map();
        const ensureClassSummary = (classId) => {
            const normalizedClassId = String(classId || '').trim().toUpperCase() || 'CHUA_PHAN_LOP';
            if (!classMap.has(normalizedClassId)) {
                classMap.set(normalizedClassId, {
            ...
            return classMap.get(normalizedClassId);
        };
        students.forEach((student) => {
            const classSummary = ensureClassSummary(student.class_id);
            ...
            const studentInfo = {
                ...
                orderNumber: student.order_number,
            };
            classSummary.totalStudents += 1;
            if (!score) {
                classSummary.notSubmittedCount += 1;
                classSummary.notSubmittedStudents.push(studentInfo);
                return;
            }
            const status = String(score.status || 'PENDING').toUpperCase();
            const scoredStudent = {
                ...studentInfo,
                scoreId: score.id,
                status,
                selfTotal: score.total,
                approvedTotal: score.admin_total,
                updatedAt: score.updatedAt,
            };
            classSummary.submittedCount += 1;
            classSummary.submittedStudents.push(scoredStudent);
            if (status === 'APPROVED') {
                classSummary.approvedCount += 1;
                classSummary.approvedStudents.push(scoredStudent);
            }
            else if (status === 'REJECTED') {
                classSummary.rejectedCount += 1;
                classSummary.rejectedStudents.push(scoredStudent);
            }
            else {
                classSummary.pendingCount += 1;
                classSummary.pendingStudents.push(scoredStudent);
            }
        });
        const classSummaries = Array.from(classMap.values())
            .map((item) => ({
            ...item,
            submissionRate: item.totalStudents > 0 ? Math.round((item.submittedCount / item.totalStudents) * 100) : 0,
            approvalRate: item.submittedCount > 0 ? Math.round((item.approvedCount / item.submittedCount) * 100) : 0,
        }))
            .sort((a, b) => a.classId.localeCompare(b.classId));
        const totals = classSummaries.reduce((acc, item) => {
            acc.totalStudents += item.totalStudents;
            acc.submittedCount += item.submittedCount;
            acc.approvedCount += item.approvedCount;
            acc.pendingCount += item.pendingCount;
            acc.rejectedCount += item.rejectedCount;
            acc.notSubmittedCount += item.notSubmittedCount;
            return acc;
        }, {
            totalStudents: 0,
            submittedCount: 0,
        ...
        return res.json({
            semester: semesterName,
            classFilter: effectiveClassId || null,
            generatedAt: new Date().toISOString(),
            totals: {
                ...totals,
                submissionRate: totals.totalStudents > 0 ? Math.round((totals.submittedCount / totals.totalStudents) * 100) : 0,
                approvalRate: totals.submittedCount > 0 ? Math.round((totals.approvedCount / totals.submittedCount) * 100) : 0,
            },
            classes: classSummaries,
        });
    }
    catch (error) {
        console.error('Error in getTrainingStatistics:', error);
        return res.status(500).json({ message: 'Server error' });
    }
};
const recalculateScoreDetails = (details) => {
    let sec1 = 0;
    let sec2 = 0;
    let sec3 = 0;
    let sec4 = 0;
    ...
    for (const [key, rawValue] of Object.entries(details)) {
        ...
        const attendanceScore = activities
            .filter((act) => act.source !== 'CUSTOM_EVIDENCE')
            .reduce((sum, act) => sum + (Number(act.points) || 0), 0);
...
export const submitStudentCustomEvidence = async (req, res) => {
...
export const getStudentCustomEvidence = async (req, res) => {
    const studentId = Number(req.user?.studentId || 0);
    if (!studentId) {
    ...
    try {
        ...
        scores.forEach((score) => {
            ...
            Object.entries(details).forEach(([critId, critVal]) => {
                if (critVal && Array.isArray(critVal.customEvidence)) {
                    critVal.customEvidence.forEach((item) => {
                        flatEvidence.push({
                            ...item,
...
export const getAllCustomEvidence = async (req, res) => {
    try {
        const scores = await prisma.trainingScore.findMany({
            include: {
                student: {
...
export const reviewCustomEvidence = async (req, res) => {
    ...
    try {
        ...
        for (const [critId, critVal] of Object.entries(details)) {
            if (critVal && Array.isArray(critVal.customEvidence)) {
                const idx = critVal.customEvidence.findIndex((item) => item.id === evidenceId);
                if (idx !== -1) {
                    foundItem = critVal.customEvidence[idx];
                    oldCritId = critId;
                    foundItem.status = status;
                    if (points !== undefined)
                        foundItem.points = Number(points);
                    if (criterionId !== undefined)
                        foundItem.criterionId = criterionId;
                    break;
                }
            }
        }
        ...
        if (status === 'APPROVED') {
            if (targetCritId !== oldCritId) {
                details[oldCritId].customEvidence = details[oldCritId].customEvidence.filter((item) => item.id !== evidenceId);
            ...
            if (!details[targetCritId].activities)
                details[targetCritId].activities = [];
            const actLog = {
            ...
            const actIdx = details[targetCritId].activities.findIndex((act) => act.evidenceId === evidenceId || (act.source === 'CUSTOM_EVIDENCE' && act.activityName === foundItem.activityName));
            if (actIdx !== -1) {
            ...
            if (details[targetCritId] && Array.isArray(details[targetCritId].activities)) {
                details[targetCritId].activities = details[targetCritId].activities.filter((act) => act.evidenceId !== evidenceId);
            }
        }
