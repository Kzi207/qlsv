...
for (const candidate of envCandidates) {
  if (loadedPaths.has(candidate) || !fs.existsSync(candidate)) continue;
...
if (!String(process.env.DATABASE_URL || '').trim()) {
  throw new Error(
    'Missing DATABASE_URL. Create backend/.env (or project-root .env) and set DATABASE_URL before starting backend.',
  );
}
