...
export const registerEvent = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (
      ...
      (error.meta as { target: unknown[] }).target.includes('eventId') &&
      (error.meta as { target: unknown[] }).target.includes('studentCode')
    ) {
...
export const exportEventRegistrationsExcel = async (req: Request, res: Response) => {
  const { id } = req.params;
